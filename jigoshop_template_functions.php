	...
	<form  method="post" action="'.esc_url($_product->add_to_cart_url()).'">
        <div class="modal-content">
            <div class="modal-header">
				...
				<h3 itemprop="name">'.$_product->get_title().'<span class="pull-right">'.$_product->get_price().' RSD</span></h3>
            </div>
...
<div id="optr_87007_4294 " class=""><hr><h4>'.translate_text('Prilozi','Прилози', ICL_LANGUAGE_CODE).'</h4><div class="masonery">
<div><label><input type="checkbox" id="pavlaka" name="dodaci[]" value="Pavlaka">'.translate_text('Pavlaka','Павлака', ICL_LANGUAGE_CODE).' <span class="muted"></span></label></div><div><label><input type="checkbox" id="zelenasalata" name="dodaci[]" value="Zelena salata">'.translate_text('Zelena salata','Зелена салата', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="kecap" name="dodaci[]" value="Kecap">'.translate_text('Kečap','Кечап', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="senf" name="dodaci[]" value="Senf">'.translate_text('Senf','Сенф', ICL_LANGUAGE_CODE).' <span class="muted"></span></label></div><div><label><input type="checkbox" id="3u1" name="dodaci[]" value="3 u 1 (Pavlaka, majonez, kecap)">'.translate_text('3 u 1 (Pavlaka, majonez, kecap)',' 3 у 1 (Павлака, мајонез, кечап)', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="kupus" name="dodaci[]" value="Kupus">'.translate_text('Kupus','Купус', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="majonez" name="dodaci[]" value="Majonez">'.translate_text('Majonez','Мајонез', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="tartar" name="dodaci[]" value="Tartar">'.translate_text('Tartar','Тартар', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="urnebes" name="dodaci[]" value="Urnebes">'.translate_text('Urnebes','Урнебес', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="tucanapaprika" name="dodaci[]" value="Tucana Paprika">'.translate_text('Tucana paprika','Туцана паприка', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div><div><label><input type="checkbox" id="luk" name="dodaci[]" value="Luk">'.translate_text('Luk','Лук', ICL_LANGUAGE_CODE).'<span class="muted"></span></label></div></div>
</div>

<div class="food">'.translate_text('Upustva','Упутства', ICL_LANGUAGE_CODE).' <span class="muted">'.translate_text('(nije obavezno)','(није обавезно)', ICL_LANGUAGE_CODE).'</span><br><textarea rows="1" cols="80" id="inputDirections" name="inputDirections"></textarea><br><span class="muted">'.translate_text('Molimo navedite eventualne posebne zahteve za pripremu ili dostavu, npr,Bez belog luka, ili za Đorđa... Dodatni zahtevi mogu povećati konačnu cenu.','Молимо наведите евентуалне посебне захтеве за припрему или доставу, нпр. "без белог лука" или "за Ђорђа"... Додатни захтеви могу повећати коначну цену.', ICL_LANGUAGE_CODE).'</span></div>


    </div>
            <div class="modal-footer">
			<button btn btn-primary" type="submit" rel="nofollow"><i class="icon icon-ok"></i>'.translate_text('Dodaj u korpu','Додај у корпу!', ICL_LANGUAGE_CODE).'</button>
...
if (!function_exists('jigoshop_show_product_images')) {
 ...
 ?><a href="<?php echo $_product->get_brand(); ?>" rel="gallery"><img alt="" width="300px" class="frame img_nofade" src="<?php echo $_product->get_brand(); ?>" style="opacity:1;" /> </a> <?php
        }
	...
	function jigoshop_verify_checkout_states_for_countries_message() {
		if ( Jigoshop_Base::get_options()->get( 'jigoshop_verify_checkout_info_message' ) == 'yes' ) {
			...
			if ( ! jigoshop_customer::has_valid_shipping_state() ) {
				...
				echo '<div class="clear"></div><div class="payment_message">' . __('Molimo vas proverite Vase podatke prebgo sto potvrdite porudzbinu.','jigoshop') . '</div>';
			}
		}
	}
}
