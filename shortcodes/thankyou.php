...
function jigoshop_thankyou() {
	...
	if ($order_id > 0) :
		...
		if ($order->order_key == $order_key) :
			...
			$order = new jigoshop_order($order->id);
			jigoshop_emails::send_mail('admin_order_status_pending_to_processing', get_order_email_arguments($order->id), $options->get('jigoshop_email'));			
			jigoshop_emails::send_mail('customer_order_status_pending_to_processing', get_order_email_arguments($order->id), $order->billing_email);
		endif;
